...
const Sudoku = () => {
  ...
  const handleSolve = () => {
    let solvedBoard = arrayCopy(grid);
  ...
  const handleNewGame = (maxEmptyCellsCount) => {
    ...
    setStartingGrid(arrayCopy(newSudokuGrid));
    setGrid(arrayCopy(newSudokuGrid));
  ...
  const handleClearBoard = () => {
    setIsPlayerWon(false);
    setGrid(arrayCopy(startingGrid));
  };
